...
if ( !class_exists( 'busconHelper' ) ) {
    class Buscon_Helper {
         ...
         */
        public static function buscon_header_template() {
            $id = get_the_ID();
            ...
            if ( $header_style && ( get_post( $header_style ) ) && in_array( 'elementor/elementor.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) {?>
                ...
                if( class_exists( 'Buscon_Core_Helper' ) ) {

                ?>
					<?php echo Buscon_Core_Helper::buscon_render_header( $header_style ); ?>
				<?php
                }
			} else {
                function_exists( 'buscon_default_header' ) ? buscon_default_header() : '';
            }
        }
         ...
         * @return  [type]  [return description]
         */
        public static function buscon_footer_template() {
            $id = get_the_ID();
            ...
            if ( $footer_style && ( get_post( $footer_style ) ) && in_array( 'elementor/elementor.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) {?>
				<?php $elementor_instance = Elementor\Plugin::instance();
                    if( class_exists( 'Buscon_Core_Helper' ) ) {
                ?>
					<?php echo Buscon_Core_Helper::buscon_render_footer( $footer_style ); ?>
				<?php
                    }
			} else {
                function_exists( 'buscon_default_footer' ) ? buscon_default_footer() : '';
            }
        }
