...
/**
 * [buscon_breadcrumb description]
 * @return [type] [description]
 */
function buscon_breadcrumb() {
    ...
    if ( is_front_page() && is_home() ) {
        ...
        $title = esc_html__( 'Search Results for : ', 'buscon' ) . get_search_query();
    } elseif ( is_404() ) {
        $title = esc_html__( 'Page not Found', 'buscon' );
    } elseif ( function_exists( 'is_woocommerce' ) && is_shop() ) {
        $title = get_the_title( get_option( 'woocommerce_shop_page_id' ) );
    } elseif ( function_exists( 'is_woocommerce' ) && is_product() ) {
        $title = __( 'Product Details', 'buscon' );
    ...
    if ( $enable_page_preadcrumb == true && $breadcrumb_show == 1 ) {
        ...
        // check if front page
        if ( is_front_page() ) {
            $title = __('Blog', 'buscon');
        }

        $enable_breadcrumb_list = cs_get_option( 'enable_breadcrumb_list', true );

        ?>
        <div class="tx-breadcrumb breadcrumb-area bg-default has-breadcrumb-overlay <?php echo esc_attr( $wpbreadcrumb_class . $shop_details_breadcrumb ); ?>" data-background="<?php echo esc_url($bg_url); ?>">
					...
					<div class="col-xxl-12">
						<div class="breadcrumb-wrap text-center">
							...
							<div class="breadcrumb-list wow fadeInUp mt-30" data-wow-duration="2s">
                                <?php buscon_breadcrumb_callback();?>
							</div>
                            <?php endif; ?>
						</div>
					</div>
...
add_action( 'buscon_before_main_content', 'buscon_breadcrumb' );

function buscon_breadcrumb_callback() {
    $args = [
        'show_browse'   => false,
        'post_taxonomy' => ['product' => 'product_cat'],
    ];
    $breadcrumb = new BusconBreadcrumbClass( $args );
