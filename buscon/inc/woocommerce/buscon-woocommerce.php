...
/**
 * [buscon_product_title description]
 * @return [type] [description]
 */
function buscon_product_title() {
    return '<h6 class="tx-product-title shop-block_one-heading"><a href="' . get_the_permalink() . '">' . get_the_title() . '</a></h6>';
}

/**
 * [buscon_product_cart_button description]
 * @return [type] [description]
 */
function woocommerce_custom_sale_text($text, $post, $_product) {
    $show_discount_percentage = cs_get_option( 'show_discount_percentage', false );
    if($show_discount_percentage == false) {
        return '<span class="tx-product-badge tx-product-badge__sale off-tag position-absolute">'.__('Sell', 'buscon').'</span>';
    }
}


function buscon_woo_category() {
    global $product;
...
function buscon_get_price() {
    ob_start(); ?>
    <span class="woocommerce-Price-amount amount price"><?php print buscon_get_price_html(); ?></span>
    <?php
    return ob_get_clean();
}

function buscon_product_price( $product_id, $oldp = false ) {
...
function buscon_get_price_html() {
    global $product;
    return $product->get_price_html();
}

/**
 * [buscon_comment_rating description]
 * @param  [type] $rating [description]
 * @return [type]         [description]
 */
function buscon_comment_rating($rating)
{
...
add_filter('woocommerce_product_additional_information_heading', 'buscon_tab_heading');

add_filter('woocommerce_product_description_heading', 'buscon_tab_heading');

/**
 * [buscon_tab_heading description]
 * @param  [type] $heading [description]
 * @return [type]          [description]
 */
function buscon_tab_heading($heading)
{
    return '';
}

function buscon_woocommerce_get_sidebar()
{
    dynamic_sidebar('product-sidebar');
}

// add to cart function
function buscon_add_cart_button() {
    global $product;
    ...
    $custom_add_to_cart_text = cs_get_option('custom_add_to_cart_text', __('Purchase Now', 'buscon'));
...
function buscon_product_actions() {

    if(function_exists('buscon_quick_view_button') && BUSCON_CORE && BUSCON_WOOCOMMERCE_ACTIVED && class_exists( 'WPCleverWoosq' )) {
    ?>
        <div class="eye">
            <?php print buscon_quick_view_button(); ?>
        </div>
        <?php
    }
}

function buscon_woocommerce_template_single_price(){
    print buscon_get_price_html();
}
...
function buscon_related_products_limit() {
    global $product;

      $args['posts_per_page'] = 3;
      return $args;
}
add_filter( 'woocommerce_output_related_products_args', 'buscon_related_products_args', 20 );
function buscon_related_products_args( $args ) {
